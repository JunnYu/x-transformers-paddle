...
class GluonBackend(AbstractBackend):
    ...
    def from_numpy(self, x):
        if len(x.shape) == 0:
            x = x[
                None
            ]  # poor support of scalars in mxnet, otherwise mxnet can't attach gradients
...
class MXNetBackend(AbstractBackend):
    ...
    def shape(self, x):
        ...
        if len(shape) == 0:
            warnings.warn(
                "mxnet inferred shape to be (), which probably means it could not be inferred"
            )
    ...
    def reshape(self, x, shape):
        ...
        if any(isinstance(dimension, UnknownSize) for dimension in shape):
            ...
            raise EinopsError(
                "Mxnet couldn't infer all dimensions statically, please provide those with axes_lengths"
            )
        return x.reshape(shape)
