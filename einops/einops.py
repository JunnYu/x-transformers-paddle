...
def _optimize_transformation(init_shapes, reduced_axes, axes_reordering, final_shapes):
    ...
    for i in range(len(reduced_axes) - 1)[::-1]:
        if reduced_axes[i] + 1 == reduced_axes[i + 1]:
            ...
            reduced_axes = reduced_axes[: i + 1] + tuple(
                axis - 1 for axis in reduced_axes[i + 2 :]
            )
    ...
    for init_axis in range(len(init_shapes) - 1)[::-1]:
        ...
        if init_axis_to_final_axis[init_axis + 1] is None:
            continue
        if (
            init_axis_to_final_axis[init_axis] + 1
            == init_axis_to_final_axis[init_axis + 1]
        ):
            removed_axis = init_axis + 1
            removed_length = init_shapes[removed_axis]
            removed_axis_after_reduction = sum(
                x not in reduced_axes for x in range(removed_axis)
            )

            reduced_axes = tuple(
                axis if axis < removed_axis else axis - 1 for axis in reduced_axes
            )
            init_shapes = init_shapes[:removed_axis] + init_shapes[removed_axis + 1 :]
            init_shapes[removed_axis - 1] *= removed_length
...
class TransformRecipe:
    ...
    def __init__(
        ...
        self.input_composite_axes: List[
        ...
        self.ellipsis_position_in_lhs: int = (
            ellipsis_position_in_lhs if ellipsis_position_in_lhs is not None else 10000
        )
...
def _apply_recipe(recipe: TransformRecipe, tensor, reduction_type: Reduction):
    ...
    if len(added_axes) > 0:
        tensor = backend.add_axes(
            tensor, n_axes=len(axes_reordering) + len(added_axes), pos2len=added_axes
        )
...
def _prepare_transformation_recipe(
    ...
    if not left.has_ellipsis and rght.has_ellipsis:
        raise EinopsError(
            "Ellipsis found in left side, but not right side of a pattern {}".format(
                pattern
            )
        )
    if left.has_ellipsis and left.has_ellipsis_parenthesized:
        raise EinopsError(
            "Ellipsis is parenthesis in the left side is not allowed: {}".format(
                pattern
            )
        )
    if operation == "rearrange":
        ...
        if len(difference) > 0:
            raise EinopsError(
                "Unexpected identifiers on the left side of repeat: {}".format(
                    difference
                )
            )
        ...
        if len(difference) > 0:
            raise EinopsError(
                "Unexpected identifiers on the right side of reduce {}: {}".format(
                    operation, difference
                )
            )
    else:
        raise EinopsError(
            "Unknown reduction {}. Expect one of {}.".format(operation, _reductions)
        )
    ...
    reduced_axes: List[int] = [
        position
        for axis, position in axis_name2position.items()
        if axis not in rght.identifiers
    ]
    ...
    ellipsis_left = (
        None if _ellipsis not in left.composition else left.composition.index(_ellipsis)
    )
...
def reduce(tensor, pattern: str, reduction: Reduction, **axes_lengths: int):
    ...
    try:
        ...
        recipe = _prepare_transformation_recipe(
            pattern, reduction, axes_lengths=hashable_axes_lengths
        )
        return _apply_recipe(recipe, tensor, reduction_type=reduction)
    except EinopsError as e:
        message = ' Error while processing {}-reduction pattern "{}".'.format(
            reduction, pattern
        )
        if not isinstance(tensor, list):
            message += "\n Input tensor shape: {}. ".format(
                get_backend(tensor).shape(tensor)
            )
        else:
            message += "\n Input is list. "
...
def parse_shape(x, pattern: str):
    ...
    names = [
        elementary_axis
        for elementary_axis in pattern.split(" ")
        if len(elementary_axis) > 0
    ]
