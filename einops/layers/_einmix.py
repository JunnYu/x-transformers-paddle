...
class _EinmixMixin:
    def __init__(self, pattern, weight_shape, bias_shape=None, **axes_lengths):
        ...
        if any(len(group) != 1 for group in left.composition):
            ...
            pre_reshape_lengths = {
                name: length
                for name, length in self.axes_lengths.items()
                if name in names
            }
        ...
        self._create_rearrange_layers(
            pre_reshape_pattern, pre_reshape_lengths, post_reshape_pattern, {}
        )
        ...
        _fan_in = _product(
            [
                axes_lengths[axis]
                for axis, in weight.composition
        ...
        mapping2letters = {
            k: letter for letter, k in zip(string.ascii_lowercase, mapping2letters)
        }
