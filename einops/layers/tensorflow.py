...
def _compute_output_shape(recipe: TransformRecipe, input_shape) -> List[Optional[int]]:
    ...
    output_shape: List[Optional[int]] = [
        None if isinstance(d, UnknownSize) else int(d) for d in final_shape
    ]
    return output_shape
...
class EinMix(_EinmixMixin, Layer):
    def _create_parameters(self, weight_shape, weight_bound, bias_shape, bias_bound):
        self.weight = tf.Variable(
            tf.random_uniform_initializer(-weight_bound, weight_bound)(
                shape=weight_shape
            ),
            trainable=True,
        )
        if bias_shape is not None:
            self.bias = tf.Variable(
                tf.random_uniform_initializer(-bias_bound, bias_bound)(
                    shape=bias_shape
                ),
                trainable=True,
            )
